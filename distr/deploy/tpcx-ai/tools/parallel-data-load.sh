...
for src in $FILES_TO_LOAD; do  
  ...
  for node in $nodes; do
    ...
    if [[ $is_dir -eq 1 ]]
    then
      #hdfs dfs -mkdir -p $destination/${file}.seq
      ssh $node "hdfs dfs -mkdir -p $destination/${file}.seq"
      #${TPCx_AI_HOME_DIR}/tools/dir2seq.sh $src ${src}.seq && hdfs dfs -copyFromLocal -t $threads -f ${src}.seq $destination/${file}.seq/"'`hostname`' &
      ssh $node "${TPCx_AI_HOME_DIR}/tools/dir2seq.sh $src ${src}.seq && hdfs dfs -copyFromLocal -t $threads -f ${src}.seq $destination/${file}.seq/"'`hostname`' &
    else
      ...
      #hdfs dfs -mkdir -p $destination/$file
      ssh $node "/tmp/hadoop/bin/hdfs dfs -mkdir -p $destination/$file"
      #hdfs dfs -copyFromLocal -t $threads -f $src $destination/$file #/"'`hostname`' &
      ssh $node "/tmp/hadoop/bin/hdfs dfs -copyFromLocal -t $threads -f $src $destination/$file/"'`hostname`' &
    fi
  done
done
